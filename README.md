...
**바로 보기: <https://sweettoon.katsuranbo.com>** — 설치 없이 독자·창작자·운영자
흐름을 그대로 확인할 수 있습니다. 과제 확인용 임시 인스턴스라 로그인이 없고
누구나 조작할 수 있으므로 데이터가 달라져 있을 수 있습니다. 시드는 멱등이라
재배포하면 초기 상태로 돌아갑니다. 재현 가능한 정본 실행은 아래
[`docker compose up --build`](#실행)입니다.
...
- **핵심 사용자 — 웹툰 독자:** 출퇴근·등하교나 휴식 시간에 모바일과 PC를
  오가며 연재작을 읽는 10~30대 독자입니다. 익숙한 웹툰 탐색과 읽기 흐름
  안에서 좋아하는 완결 시즌을 실물 소장본으로 남기고 싶어 합니다. 로그인
  없는 데모 특성상 읽던 위치 이어보기는 같은 브라우저 안에서만 동작합니다.
...
로그인 없는 공용 데모이므로 `/orders`는 이 브라우저에서 만든 주문의 복귀
경로이고, 공용 주문 피드는 `/orders/demo`로 분리되어 있습니다. 두 화면과
`/operations` 이하가 개인 계정 화면이 아님을 UI에 명시하고 검색 색인에서도
제외했습니다. 실제 서비스에서는 인증과
...
3. 무료 공개 범위는 세로 스크롤 또는 양면 보기로 감상하고 책갈피를 남깁니다.
   마음에 드는 회차에는 좋아요를 남길 수 있고, 집계는 쇼츠 카드와 본편 뷰어가
   공유합니다.
4. 잠긴 회차는 캔디 1개로 해금합니다. 완결된 시즌의 권은 판형·표지·수량을
   선택한 소장본 Mock 주문으로도 해금할 수 있고, 연재 중인 시즌은 완결 후
   주문할 수 있다는 안내를 보여줍니다.
...
- **추가 구현:** SEO, 이미지 검증·WebP 파생,
  ...
  Mock 계약, 찜·개인화 추천, 회차 좋아요, 주문 SSE·실시간 독자 presence와
  데모 봇, 공개·무료 첫 회차를 두 페이지씩 자동 재생하는 랜덤 웹툰 쇼츠
  (배치 사전 로딩·마지막 카드 자동 연결·작가와 장르 반복 완화),
  자동화된 Compose/E2E 검증
...
- `/shorts` — 무료 첫 회차를 두 페이지씩 보여주는 랜덤 웹툰 쇼츠. 남은 카드가
  ...
  추가 입력 없이 이어서 재생한다. 오래 감상해도 마운트되는 카드는 활성 카드
  주변 20장으로 제한하고, 창 밖 카드는 자리만 남겨 스크롤 위치를 보존한다
...
- 공개 데모(<https://sweettoon.katsuranbo.com>)는 로그인 없이 모든 흐름을 확인할
  수 있도록 `API_SECURITY_MODE=demo`로 운영합니다. 따라서 링크를 아는 사람은
  누구나 회차를 발행하거나 주문 상태를 바꿀 수 있고, 데이터가 초기 시드와 다를
  수 있습니다. 실제 운영에서는 인증과 `strict` 모드 API key 경계를 함께 적용해야
  합니다. 재현 가능한 상태 확인은 로컬 `docker compose up --build`를 사용합니다.
